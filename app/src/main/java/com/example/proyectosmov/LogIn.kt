    ...
    private fun setup(){
        ...
        botonAcceder.setOnClickListener {
            ...
            if (company.text.isNullOrEmpty() or email.text.isNullOrEmpty() or password.text.isNullOrEmpty()) {
                ...
                db.collection("companies").whereEqualTo("name", company.text.toString()).limit(1) .get().addOnCompleteListener { task ->
                    if (task.isSuccessful and (task.result?.isEmpty == false)) {
                        for (documentSnapshot in task.result!!.documents) {
                            val companyId = documentSnapshot.getId()
...
//                                                )
                                                .signInWithEmailAndPassword(
                                                    "administrador.instancias@organiza2.com",
                                                     "admin123456"
                                                )
                                                .addOnCompleteListener {
                                                    if (it.isSuccessful) {
                                                        showMenu(
//                                                            it.result?.user?.email ?: "", companyId
                                                            email.text.toString(), companyId
